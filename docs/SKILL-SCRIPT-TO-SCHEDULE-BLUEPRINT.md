...
不要把显示名当作 ID，不要因为“前一天已对话”这种自然语言描述就擅自发明不存在的公共变量或完成条件。只有当项目数据契约提供了真实、稳定的条件时，才编码 `prerequisites` 或蓝图条件。
...
修复：创建或复用活动实例，让 `ScheduleRunner` 执行 `consumeTime`；应用层只负责入口、展示和实例生命周期。
