...
| `ScheduleData` | 读取活动文件、维护动态插入、按时间追加实例 |
...
| `ItemScheduleRuntime` | 物品和其他非阻塞操作的活动运行时 |
...
- `insertSchedule`：通过 `ScheduleData` 动态插入活动，不能直接写 queue；可用 `respectPrerequisite` 忽略先决条件，或用 `protectFromExpiry` 保护实例免于过期。
...
`ScheduleRunner` 每遇到 `text` 节点就暂停；有 UI 容器时等待明确的继续动作，遇到 `choice` 时等待选项。无 UI 容器的 headless realtime 活动必须自动继续，不能因为没有按钮而死锁。
