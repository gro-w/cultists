...
| 活动来源与动态追加 | `ScheduleData` | `fired`、`pendingAdds`、`lastAbsoluteMinute`、动态活动请求及其目标队列 | 无 |
...
14. **`queuedAdds`**：动态活动的 `scheduleId`、`addTime`、可选 `queueId`。
...
1. `ScheduleData.fired`、`pendingAdds`、`lastAbsoluteMinute` 尚未在“活动与队列”中单独展示。
...
1. 在“活动与队列”中补充 `ScheduleData.fired`、`pendingAdds` 和
