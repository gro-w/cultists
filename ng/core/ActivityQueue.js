...
export class ScheduleQueue {
  ...
  complete(id, result = null) { const entry = this.getInstance(id); if (!entry) throw new Error(`Unknown schedule instance: ${id}`); entry.transition("resolved", result); this.activeInstanceIds = this.activeInstanceIds.filter((value) => value !== id); return entry; }
  ...
  restore(snapshot) { this.entries = (snapshot.entries || []).map((entry) => ScheduleInstance.restore(entry)); this.activeInstanceIds = [...(snapshot.activeInstanceIds || [])]; }
}
