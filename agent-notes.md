...
成就、物品、患者、宿舍、日历、室友剧情、结局、应用和患者队列属于 Cultists 内容层，而不是引擎内置业务。业务由 `content/` 内容包中的数据结构、自定义窗口和自定义 Activity/管理器 Activity 组成。

`core/engine.js` 是平台入口，负责加载内容包并启动平台。内容包组合引擎提供的通用桌面、窗口、Activity、时间推进和存档 API。
