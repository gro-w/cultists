...
import { ScheduleListManager } from "./ScheduleListManager.js";
...
export class DeveloperMode {
  constructor({ definitions, validator, variables, structures, windowManager, store, state, queues, nodes, scheduleLists } = {}) { Object.assign(this, { definitions, validator, variables, structures, windowManager, store, state, queues, nodes, scheduleLists }); this.root = null; }
  ...
  openBlueprint(host) {
    if (!this.definitions.list().length) return host.textContent = "没有已加载的 Schedule。";
    const manager = new ScheduleListManager({ definitions: this.definitions, scheduleLists: this.scheduleLists, onOpen: (definition) => this.openScheduleEditor(definition), onSave: (list) => this.saveScheduleList(list) });
    manager.mount(host);
  }
  openScheduleEditor(definition) {
    const host = document.createElement("div");
    const frame = this.windowManager.openDynamic({ id: `ng-schedule-${definition.id}`, title: `蓝图：${definition.displayName || definition.id}`, content: host, width: 760, height: 680 });
    const editor = new ScheduleEditor({ definition, nodes: this.nodes, validator: (value) => this.validator.validate(value), writeToDisk: (file, value) => this.writeToDisk(file, value), fileScope: { fileName: `schedules/${definition.id}.json`, type: "schedule" }, fileLabel: definition.displayName || definition.id, onChange: (value) => { this.definitions.replace(value); this.status(`Schedule ${value.id} 已更新到内存。`); } });
    host.innerHTML = editor.html(); editor.mount(host.querySelector(".dev-de-root") || host); return frame;
  }
  async saveScheduleList(list) {
    const file = `schedule-lists/${list.id}.json`;
