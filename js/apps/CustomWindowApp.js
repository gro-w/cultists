...
import { scheduleEffectExecutor } from "../core/ScheduleEffectExecutor.js";
...
export async function launchCustomWindowApp(appId = "his_custom") {
  ...
  await Promise.all([scheduleData.init(), medicalCaseManager.load()]);
  ...
  const uiEffects = {
    ...
    hisSelectPatient: (patientId) => {
      ...
      renderPrescription();
      const dialogue = widget("dialogue");
      if (activePatientRunner) scheduleExecutionService.cancel(activePatientRunner.instance?.instanceId);
      if (!dialogue || !entry.queueEntry || entry.queueEntry.status === "resolved") return;
      const lines = document.createElement("div");
      const controls = document.createElement("div");
      dialogue.replaceChildren(lines, controls);
      activePatientRunner = scheduleExecutionService.run({
        queue: workQueue,
        definition: entry,
    ...
    hisSubmit: (diagnosisId, medicineIds) => {
      ...
      const result = scheduleEffectExecutor.submitMedical(selectedPatient, diagnosisId, Array.isArray(medicineIds) ? medicineIds : []);
      const submit = widget("submit");
  ...
  const runBlueprint = (target, eventName, inputValues = {}) => {
    ...
    const result = scheduleData.createTemporaryInstance(clone(blueprint), "main", {});
    if (!result.ok) return null;
    const instance = result.instance;
    return scheduleExecutionService.run({
      queue: mainQueue,
  ...
  const offSchedule = eventBus.on("schedule:appended", ({ queueId }) => { if (queueId === "work") renderPatients(); });
