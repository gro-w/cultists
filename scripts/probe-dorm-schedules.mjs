...
assert.match(dormMode, /_showScheduleDialogue\(definition, item\)/, "schedule button must reuse the schedule dialogue runner");
...
const schedules = social.filter((entry) => entry.id.startsWith("dorm_schedule_day"));
assert.equal(schedules.length, 4, "four weekday dorm schedules expected");
const byDay = new Map(schedules.map((entry) => [entry.day, entry]));
for (let day = 1; day <= 4; day += 1) {
  const entry = byDay.get(day);
  assert.ok(entry, `missing dorm schedule day ${day}`);
  const bp = entry.blueprint;
  assert.equal(bp.nodes.prerequisite.inputs.condition, true);
  assert.ok(bp.nodes.choice, `day ${day} needs study/schedule choice`);
  assert.ok(bp.nodes.choice.options[1].condition.any, `day ${day} schedule option needs favorability gate`);
  ...
  assert.equal(bp.nodes.schedule_set.inputs.variableId, 103 + day);
  assert.ok(Object.values(bp.nodes).some((node) => node.type === "scheduleExpiry"));
}
const day2 = byDay.get(2).blueprint;
assert.equal(day2.nodes.day1_schedule.inputs.variableId, 104);
const day3 = byDay.get(3).blueprint;
assert.equal(day3.nodes.previous_schedule.inputs.variableId, 105);
assert.deepEqual(
...
for (const [day, file] of [[1, "social01b.json"], [2, "social02b.json"], [3, "social03b.json"], [4, "social04b.json"]]) {
  ...
  assert.ok(entries.some((entry) => entry.blueprint?.nodes?.insert_dorm_schedule?.inputs?.scheduleId === `dorm_schedule_day${day}`), `day ${day} invitation insertion missing`);
}
const day5 = read("social05a.json").entries;
assert.equal(day5.length, 1);
assert.equal(day5[0].id, "dorm_schedule_day5");
...
console.log("dorm schedule probe: ok");
