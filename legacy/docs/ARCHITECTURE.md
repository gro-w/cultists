...
常见事件包括：`gamestate:changed`、`time:changed`、`daynight:changed`、`activity:requested`、`activity:resolved`、`activity:completed`、`item:inspected`、`item:used`、`spells:changed`、`spell:cast`、`global-variable:changed`、`global-variables:changed`、`ending:triggered`。`activity:requested` 是领域生产者发出的命令请求，由 `ActivityTriggerRouter` 路由到统一的实时活动执行器；`activity:triggered` 仅作为旧接口常量保留，不再作为新的生产者入口。
...
| 物品调查/使用、法术施放、NPC 离线 | `mainQueue` | 领域生产者发出 `activity:requested`，`ActivityTriggerRouter` 路由到 `ItemActivityRuntime`，再由 `ActivityExecutionService` 统一管理生命周期 |
