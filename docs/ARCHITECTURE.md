...
| `ScheduleData` | 加载 work/social 活动，并按时间点追加批次 |
...
| `ScheduleRunner` | HIS/Social 对话及所有对象式活动蓝图执行器 |
...
常见事件包括：`gamestate:changed`、`time:changed`、`daynight:changed`、`schedule:requested`、`schedule:resolved`、`schedule:completed`、`item:inspected`、`item:used`、`spells:changed`、`spell:cast`、`global-variable:changed`、`global-variables:changed`、`ending:triggered`。`schedule:requested` 是领域生产者发出的命令请求，由 `ScheduleTriggerRouter` 路由到统一的实时活动执行器；`schedule:triggered` 仅作为旧接口常量保留，不再作为新的生产者入口。
...
| 物品调查/使用、法术施放、NPC 离线 | `mainQueue` | 领域生产者发出 `schedule:requested`，`ScheduleTriggerRouter` 路由到 `ItemScheduleRuntime`，再由 `ScheduleExecutionService` 统一管理生命周期 |
| HIS 诊断提交 | `mainQueue` | 提交医疗记录、推进 20 分钟、完成实例 |
| 法术学习 | `mainQueue` | `consumeTime(240)` 后执行 `spellOperation` |

`TimeService` 是唯一的普通时间推进 owner。睡眠、醒来、下班、跨日、日结和最终结局是显式系统边界；医疗到期由独立的 `MedicalClockCoordinator` 观察 `time:changed` 并调用 `MedicalCaseManager.processDue()`，日结由同一协调器处理，医疗管理器不得再通过 `daynight:changed` 自行执行该逻辑。EventBus listener 若改变状态或消耗时间，必须视为执行器审计，不能当作被动通知。
