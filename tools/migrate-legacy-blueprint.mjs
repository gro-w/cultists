 ...
 * but with node `type`/`inputs` renamed to `core/ActivityNodeRegistry.js`
 * equivalents). The flat `connections` array itself needs NO transform —
 * `core/ActivityValidator.js#normalizeBlueprint()` already folds that
 ...
 *   - CLI `node tools/migrate-legacy-blueprint.mjs --report [dir]` —
...
if (isMain) {
  ...
  if (args[0] === "--report") {
    ...
    console.log("Usage: node tools/migrate-legacy-blueprint.mjs --report [dir] | --write [dir]");
    process.exit(1);
  }
}
