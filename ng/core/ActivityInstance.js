export class ScheduleInstance {
  constructor({ instanceId, definitionId, definitionVersion = 1, queueId = "main", inputs = {}, payload = {} } = {}) {
    if (!definitionId) throw new Error("Schedule definitionId is required");
    this.instanceId = instanceId || `schedule-${globalThis.crypto?.randomUUID?.() || `${Date.now()}-${Math.random()}`}`;
  ...
  static restore(snapshot) { const instance = new ScheduleInstance(snapshot); Object.assign(instance, structuredClone(snapshot)); return instance; }
  transition(status, result = null) { if (!["unresolved", "running", "blocked", "resolved", "failed", "cancelled"].includes(status)) throw new Error(`Invalid schedule status: ${status}`); if (this.terminalEmitted) return false; this.status = status; if (["resolved", "failed", "cancelled"].includes(status)) { this.terminalEmitted = true; this.result = result; } return true; }
}
