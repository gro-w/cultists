...
1. 仿 Windows 95 的桌面外壳
   ...
   - 任务栏/窗口按钮
2. 仅开发版可见的开发人员模式
3. 基于蓝图的通用 Schedule 系统
4. Schedule 列表管理器与 Schedule 可视化编辑器
...
│   ├── ScheduleDefinitionStore.js
│   ├── ScheduleQueue.js
│   ├── ScheduleQueueRegistry.js
│   ├── ScheduleInstance.js
│   ├── ScheduleExecutionService.js
│   ├── ScheduleRunner.js
│   ├── ScheduleNodeRegistry.js
...
5. 创建公共变量、数据结构、Schedule、队列、窗口、图标和存档服务
6. 恢复存档；若没有存档，创建全新引擎状态
7. 先创建窗口/桌面基础设施，再激活 Schedule 系统
8. 将唯一的 `default` Schedule 列表中的 `default` 活动自动推入 `main` 主要队列
9. 执行 `default` 活动，完成用户自定义初始化
10. 若开发入口开启，挂载开发人员模式桌面图标与工具窗口
11. 发布 `engine:ready`

初始化过程分为 `loading`、`restoring`、`activating`、`ready` 四个状态。恢复失败必须释放所有恢复锁并显示明确错误，不得继续用半恢复状态启动 Schedule。
...
| Schedule 定义 | `ScheduleDefinitionStore` | 是/由数据文件管理 |
| Schedule 实例 | `ScheduleExecutionService` | 是 |
| Schedule 队列 | `ScheduleQueueRegistry` | 是 |
...
UI 只发命令或创建 Schedule，不直接修改跨模块状态。模块间变化通过明确的事件名和 payload 传播。
...
桌面是 presentation 层。打开窗口、拖动窗口、调整大小、改变焦点不推进游戏时间，不改变游戏阶段，不触发 Schedule。任何需要推进时间的桌面图标行为都必须通过该图标绑定的内置 blueprint 执行，而不是由桌面或窗口代码直接改时间。
...
每个窗口和组件支持可选内嵌 Schedule blueprint：
...
生命周期 Schedule 必须统一经过 `ScheduleExecutionService`，不能由窗口自己创建另一套 Runner。销毁路径必须幂等，标题栏关闭、系统菜单关闭和程序化关闭只能执行一次 `onDestroy`。
...
| 对象 | 作用 |
| --- | --- |
| Schedule 定义 | 不可变或版本化的蓝图模板 |
| Schedule 列表 | 按稳定 ID 组织多个 Schedule 定义，支持时间加载 |
| Schedule 实例 | 一次实际执行的 identity、输入、游标、状态和结果 |
| Schedule 队列 | 按规则保存实例，维护阻塞、顺序和待执行项 |
...
3. 创建新的 Schedule 实例
4. 推入 `main` 主要队列
5. 通过 `ScheduleExecutionService` 执行
...
每个节点类型通过 `ScheduleNodeRegistry` 注册，声明：
...
队列管理器不能把工作、社交、医疗等领域写死。`ScheduleQueueRegistry` 负责注册和恢复，领域政策仅订阅通用生命周期事件。
...
每个 Schedule 事件都携带 `instanceId` 和 `queueId`；需要时携带 `definitionId`、`definitionVersion` 和 `cause`。事件名和 payload 由常量模块统一导出。

---

## 6. Schedule 列表管理器与可视化编辑器

### 6.1 列表管理器

开发窗口“Schedule 列表管理器”负责：
...
- 打开列表内 Schedule
- 新建、复制、删除 Schedule
- 标记 Schedule 是否按时间加载、是否自动运行
- 保存到内存
- 下载 JSON
- 通过本机开发 API 写入已存在数据文件
- 显示“从列表移除”和“删除文件”的区别

列表的左侧只显示当前列表文件和条目；未选中节点时，右侧编辑的是当前 Schedule 的 `id`、`displayName` 和元数据，不是列表文件 ID。
...
```text
桌面双击“下班”图标
  -> 创建图标绑定的内置 Schedule blueprint 实例
...
- blueprint 可以通过通用节点打开窗口、打开全屏自定义窗口、推进时间、入队 Schedule 或发出通用事件
...
- `desktop.run-schedule`
- `desktop.emit-event`

内置 blueprint 的节点仍然由通用 Schedule Runner 执行。窗口 ID、时间分钟数和其他行为参数通过输入端口传入。游戏内容可以绑定内置 blueprint，也可以在允许的范围内创建自己的图标 blueprint，但桌面组件不根据图标名称猜测行为。
...
`objectRef` 必须指定目标对象域：活动实例、Schedule 队列、自定义结构记录、公共变量或窗口实例。
...
API 需要输入 schema 校验，返回值进入 Schedule 局部变量或公共变量。不能在 UI 中直接改数据库绕过 API。
...
病人对话存储在单独 Schedule 列表，不混入医疗结构。医疗 Schedule 通过时间字段和 `blockUntil` 加载：

```text
读取 medicalAppointments
  -> blockUntil appointment.start
  -> 创建 patientDialogue Schedule
  -> 执行对话节点
  -> blockUntil 下一时间或事件
  -> 更新 medicalCase 状态
```
...
- 自定义数据库记录
- 自定义结构版本引用
- Schedule 实例、当前节点、局部变量、等待条件、结果和错误
- Schedule 队列 entries、active IDs、阻塞状态
...
- 条件/效果节点
- 物品结构、库存数据库、物品蓝图示例
- 医疗结构、病例数据库、预约和对话 Schedule 列表示例
...
- 新建、保存、刷新、加载后状态一致
- 等待中的 Schedule、窗口几何、数据库记录和 object reference 一致
- 错误存档不破坏当前状态
...
- 普通 UI 行为不会自动推进游戏时间
- 所有可见计时操作先创建 Schedule
- Schedule 只能由 ExecutionService 管理 Runner 生命周期
- 一个 Schedule 实例只有一个终态
...
措施：物品和医疗先写成结构/数据库/Schedule 示例；代码审查禁止新增 `if (item)`、`if (medical)` 这类通用运行时专用分支。
...
2. 接受内置唯一 `default` Schedule 列表及 `default/default` 自动入 `main` 队列
3. 接受 Schedule、窗口组件生命周期统一走同一 `ExecutionService`
