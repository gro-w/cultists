...
const registry = fs.readFileSync("legacy/js/core/ActivityNodeRegistry.js", "utf8");
const runner = fs.readFileSync("legacy/js/core/ActivityRunner.js", "utf8");
const activityData = fs.readFileSync("legacy/js/core/ActivityData.js", "utf8");
const hisApp = fs.readFileSync("legacy/js/apps/HISApp.js", "utf8");
const medicalManager = fs.readFileSync("legacy/js/core/MedicalCaseManager.js", "utf8");
const data = JSON.parse(fs.readFileSync("legacy/data/zh-hans/workpub.json", "utf8"));
...
assert.ok(complaint, "public complaint work activity missing");
assert.ok(riot, "public riot work activity missing");
assert.notEqual(complaint.id, riot.id);
assert.ok(!fs.existsSync("legacy/data/zh-hans/medical_events.json"), "legacy medical event data must be removed");
...
assert.ok(!activityData.includes("explain.options[0]"), "medical enqueue must not use legacy choice options");
assert.ok(activityData.includes("medical_riot_work"), "riot activity selection missing");
...
assert.ok(!nodes.death, "complaint activity should not contain riot-only death node");
assert.equal(Object.values(nodes).filter((node) => node.type === "activityEnd").length, 1);
...
console.log("medical activity migration probe: ok");
