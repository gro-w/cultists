...
完整蓝图还可选添加一个 `scheduleExpiry` 节点，最多一个。它没有流程引脚，数值输入为 `expires`（是否会过期）和 `expiresAt`（绝对游戏分钟）。节点不存在或 `expires` 不为严格 `true` 时实例永不过期；否则当前游戏时间超过 `expiresAt` 后，未解决实例会被强制标记为 `resolved`。
