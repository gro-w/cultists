...
const controls = {
  ...
  __schedule_expiry__: { id: "__schedule_expiry__", type: "scheduleExpiry", inputs: { expires: false, expiresAt: 0 }, outputs: {} },
};
...
function makeBlueprint(n) {
  ...
  for (let index = 0; index < n; index += 1) {
    ...
    nodes[endId] = { id: endId, type: "scheduleEnd", inputs: {}, outputs: {} };
    connections.push(
...
assert.equal(getScheduleNodePort("randomBranch", "flowIn", "input", randomNode).kind, "flow");
assert.equal(getScheduleNodePort("randomBranch", "n", "input", randomNode).type, "number");
assert.deepEqual([0, 1, 2].map((index) => getScheduleNodePort("randomBranch", `flowOut${index}`, "output", randomNode)?.name), ["flowOut0", "flowOut1", "flowOut2"]);
assert.equal(getScheduleNodePort("randomBranch", "flowOut3", "output", randomNode), null);
...
for (const [n, expectedIndexes] of [[1, [0]], [3, [0, 1, 2]]]) {
  ...
  for (const [randomValue, expectedIndex] of [[0, expectedIndexes[0]], [0.4, expectedIndexes[Math.min(1, expectedIndexes.length - 1)]], [0.999999, expectedIndexes.at(-1)]]) {
    ...
    new ScheduleRunner({ definition: { id: `random-${n}`, blueprint }, instance, random: () => randomValue }).start();
...
const invalidRunner = new ScheduleRunner({ definition: { id: "random-invalid", blueprint: missingN }, instance: invalidInstance, random: () => 0 });
