...
async function boot() {
  ...
  const definitions = new ScheduleDefinitionStore();
  const nodes = createDefaultNodeRegistry();
  const validator = new ScheduleValidator(nodes);
  const scheduleLists = await Promise.all((config.scheduleLists || ["default"]).map(async (id) => store.loadJSON(`schedule-lists/${id}.json`)));
  for (const scheduleId of config.schedules || []) { const definition = await store.loadJSON(`schedules/${scheduleId}.json`); validator.assert(definition); definitions.register(definition); }
  const queues = new ScheduleQueueRegistry(eventBus);
  queues.register({ queueId: "main", displayName: "主要队列", mode: "serial", autoStart: true });
  const effects = { setVariable: (node) => variables.set(node.data.variableId, node.data.value), consumeTime: (node) => { engineState.gameTime += Number(node.data.minutes || 0); }, emitEvent: (node) => eventBus.emit(node.data.event, { source: "schedule" }) };
  const execution = new ScheduleExecutionService({ definitions, queues, nodes, eventBus, effects });
  ...
  schemas.register("schedule", (value) => validator.validate(value));
  // DEV-TOOLS:START
  const openDeveloperWindow = async () => {
    ...
    try { const { DeveloperMode } = await import("./editors/DeveloperMode.js"); new DeveloperMode({ definitions, validator, variables, structures, windowManager, store, state: engineState, queues, nodes, scheduleLists }).mount(host); } catch (error) { host.textContent = `开发人员模式加载失败：${error.message}`; console.error(error); }
    return frame;
  };
  if (developerMode) {
